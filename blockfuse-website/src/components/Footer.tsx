...
const images = import.meta.glob('../assets/images/profiles/*.{png,jpg,jpeg}', { eager: true });

const allProfiles = Object.keys(images).map((path, index) => ({
  id: index,
...
const Footer = () => {
  ...
  const profilesPerBatch = 9; 
  let batchIndex = 0;

  React.useEffect(() => {
   
    const interval = setInterval(() => {
      setCurrentProfiles(allProfiles.slice(batchIndex * profilesPerBatch, (batchIndex + 1) * profilesPerBatch));
      batchIndex = (batchIndex + 1) % Math.ceil(allProfiles.length / profilesPerBatch);
    }, 5000);

    
    setCurrentProfiles(allProfiles.slice(0, profilesPerBatch));
...
<div className="mb-12 py-8 overflow-hidden sm:block">
  <div className="flex justify-center items-center -space-x-3 px-2 sm:px-4">
    {currentProfiles.map((profile, index) => {
      const centerIndex = Math.floor(currentProfiles.length / 2);
      const distanceFromCenter = Math.abs(centerIndex - index);
      const scale = 1 + (centerIndex - distanceFromCenter) * 0.08;
      ...
      return (
        ...
        >
          ...
          <div className="relative w-12 h-12 sm:w-16 sm:h-16 lg:w-24 lg:h-24 rounded-full border-2 border-purple-500 overflow-hidden">
            <img
              src={profile.image}
