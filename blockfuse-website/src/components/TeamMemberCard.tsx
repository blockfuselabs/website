...
const TeamMemberCard: React.FC<TeamMemberProps> = ({ member, navigate }) => (
  <div
    onClick={() =>
      navigate(`/teamdetails/${member.slug}`, { state: { member } })
    }
    className="flex flex-col items-center cursor-pointer transition-transform duration-200 transform hover:scale-105"
  >
    <div className="w-full h-72 sm:h-70 lg:h-70  overflow-hidden shadow-lg">
      <img
        src={member.image}
        alt={member.fullname}
        loading="lazy"
        className="w-full h-52 object-fit md:w-3/4 md:h-52 mt-3 mx-auto"
      />
